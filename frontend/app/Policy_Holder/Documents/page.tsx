...
export default function PolicyHolderDocuments() {
  ...
  const groupedClaimsList = claims.map(claim => {
    ...
    // 3. Additional Documents (Police Report, Repair Estimate, Garage Estimate Report, etc. - only show User uploaded ones)
    ...
    // 4. Garage Estimate from AI Forensic comparison (if not already included)
    if (claim.garageEstimateComparison?.garageDocumentUrl) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 md:px-16 py-10 relative z-20">
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="p-3 bg-red-100/70 text-red-600 rounded-2xl flex-shrink-0 animate-pulse">
                      <HugeiconsIcon icon={AlertCircleIcon} className="w-8 h-8" strokeWidth={2.2} />
                    </div>
        ...
        <section>
          <div className="flex items-center gap-2.5 mb-6 select-none">
            <HugeiconsIcon icon={FileCheckIcon} className="w-6 h-6 text-slate-700 flex-shrink-0" strokeWidth={2.5} />
            <h2 className="text-xl md:text-[24px] font-bold text-slate-800 tracking-tight">
              {t.uploadedDocs}
            </h2>
          </div>
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="w-10 h-10 rounded-full bg-emerald-50 border border-emerald-300 text-emerald-500 flex items-center justify-center flex-shrink-0">
                      <HugeiconsIcon icon={Tick01Icon} className="w-5 h-5" strokeWidth={3} />
                    </div>
            ...
            {!uploadSuccess ? (
              <form onSubmit={handleUploadSubmit} className="flex flex-col">
                ...
                <div className="flex flex-row justify-end items-center gap-3 mt-10">
                  ...
                  <button
                    ...
                    className="bg-[#000080] hover:bg-[#000066] disabled:bg-slate-400 active:scale-[0.97] text-white font-semibold text-sm px-10 py-3.5 rounded-full transition-all duration-150 cursor-pointer border-none flex items-center justify-center gap-2 shadow-sm"
                  >
                    {isUploading ? (
                      <>
              ...
              <div className="flex flex-col items-center select-none text-center py-6 transition-all duration-300">
                ...
                <button
                  ...
                  className="mt-8 bg-[#000080] hover:bg-[#000066] active:scale-[0.97] text-white font-semibold text-[14px] px-10 py-4 rounded-full transition-all duration-150 shadow-md border-none cursor-pointer"
                >
                  Done
                </button>
              </div>
