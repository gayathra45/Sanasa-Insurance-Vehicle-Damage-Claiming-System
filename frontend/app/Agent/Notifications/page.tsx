          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              {filteredNotifs.map((n) => {
                ...
                let iconSvg = (
                  <HugeiconsIcon icon={Time02Icon} className="w-5 h-5" strokeWidth={2} />
                );
                        ...
                        <div className="flex items-center gap-2 order-2 md:order-1">
                          {n.actionLabel === "View" ? (
                            <button
                              ...
                              className="w-[136px] md:w-[140px] h-[34px] flex items-center justify-center font-semibold text-xs rounded-full transition-all duration-150 active:scale-[0.98] cursor-pointer border-none bg-[#000080] hover:bg-[#000066] text-white shadow-xs whitespace-nowrap shrink-0"
                            >
                              View
                            </button>
                          ) : (
              ...
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={step.num} className="relative z-10 flex flex-col items-center flex-1">
                    <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center text-[14px] font-semibold relative z-10 ${circleClass}`}>
                      {isCompleted ? (
                    ...
                    <span className={`text-[11px] font-semibold mt-2 leading-none relative z-10 ${isActive ? "text-blue-600 font-semibold" : isCompleted ? "text-emerald-700 font-bold" : "text-slate-400"}`}>
                      {step.label}
                    </span>
                  </div>
                );
            ...
            <div className="bg-white border border-slate-200 rounded-[24px] w-full max-w-[720px] max-h-[90vh] shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col relative transition-all duration-300 overflow-hidden">
                      ...
                      {selectedClaim.messages.map((msg, index) => (
                        <div key={index} className="bg-slate-50 border border-slate-200/60 rounded-2xl p-4 flex flex-col gap-1 shadow-sm">
                          <div className="flex justify-between items-center text-[11px] select-none">
                            <span className="font-bold text-[#0f2d3a]">{msg.sender}</span>
              ...
              <div className="px-8 py-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center flex-shrink-0">
                <Link
                  ...
                  className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs px-6 py-3 rounded-full transition-all duration-150 no-underline shadow-sm"
                >
