...
export default function AdminNavbar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-admin-mobile-menu", handleOpen);
    return () => window.removeEventListener("open-admin-mobile-menu", handleOpen);
  }, []);
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-[280px] bg-[#102A43] h-screen sticky top-0 flex-col text-white shadow-xl shrink-0 select-none z-20">
        {/* Logo Section */}
        ...
        <nav className="flex-1 mt-6 flex flex-col">
          {menuItems.map((item) => {
            ...
            const hasChevron = ["policy holders", "agents", "staff", "admins", "analytics & reports", "claims", "registrations", "reports"].includes(item.name.toLowerCase());
      ...
      {isMobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer content */}
          <aside className="relative w-[280px] max-w-xs bg-linear-to-b from-[#111c2a] via-[#102a43] to-[#09111b] border-r border-white/10 h-full flex flex-col text-white shadow-2xl p-5 select-none animate-in slide-in-from-left duration-200">
            {/* Close Button */}
            <button
              onClick={() => setIsMobileOpen(false)}
