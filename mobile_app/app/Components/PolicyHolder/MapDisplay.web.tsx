...
<body>
  ...
  <script>
    ...
    function sendLocation(newLat, newLon) {
      window.parent.postMessage(JSON.stringify({ latitude: newLat, longitude: newLon }), '*');
    }
...
export default function MapDisplay({ latitude, longitude, onLocationSelect }: MapDisplayProps) {
  const initialCoords = React.useRef({ latitude, longitude });
  const htmlContent = React.useRef(getHtmlContent(initialCoords.current.latitude, initialCoords.current.longitude)).current;
